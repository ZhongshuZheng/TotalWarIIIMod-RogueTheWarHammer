...
├── Docs/                          ← 设计规范文档（阶段A–E、代码框架、数据再生成指南等）
...
- `generate_stage_d_data.py`：从 `OriginalGameData/db` 读取原始数据，输出 `adamrogue_data_*.lua` 等
- `update_hero_reward_metadata.py`：英雄奖励 db / 本地化补充（需在主生成器之后运行）
- `regenerate_stage_d_data.ps1`：运行入口脚本

游戏版本更新后的完整操作流程见 `Docs/adamrogue_游戏更新数据再生成指南.md`。
